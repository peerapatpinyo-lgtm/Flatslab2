...
def draw_plan_view(L1_l, L1_r, L2_t, L2_b, c1_cm, c2_cm, col_loc, has_drop, d_w1, d_w2, cant_params, edge_beam_params):
    ...
    # --- 1. FIXED BOUNDARY LOGIC ---
    slab_L = L1_l if L1_l > 0 else c1_m/2
    slab_R = L1_r if L1_r > 0 else c1_m/2
    ...
    if min_span < 1.0: # Valid span check
        valid_spans = [x for x in [L1_l, L1_r, L2_t, L2_b] if x > 0]
        min_span = min(valid_spans) * 2 if valid_spans else 4.0
    ...
    # --- EDGE BEAM (CORRECTED LOGIC) ---
    if edge_beam_params and edge_beam_params.get('has_beam'):
        beam_w = edge_beam_params['width_cm'] / 100.0
        beam_color = '#C0392B' # Dark Red
        
        # Logic: วาด Beam ตามขอบที่เป็น "Edge" จริงๆ (คือด้านที่ L=0)
        
        # 1. Left Edge Beam (Vertical) -> ถ้า L1_l เป็น 0 (Edge Col)
        if L1_l < 0.1: 
            # วาดแนวตั้งที่ขอบซ้าย
            beam_rect = patches.Rectangle((-slab_L, -slab_B), beam_w, slab_B + slab_T, 
                                          facecolor='none', edgecolor=beam_color, linestyle='--', linewidth=1.5, zorder=3)
            ax.add_patch(beam_rect)
            ax.text(-slab_L + beam_w/2, 0, "EDGE BEAM", color=beam_color, fontsize=8, ha='center', va='center', rotation=90, fontweight='bold')

        # 2. Right Edge Beam (Vertical) -> ถ้า L1_r เป็น 0
        if L1_r < 0.1:
            beam_rect = patches.Rectangle((slab_R - beam_w, -slab_B), beam_w, slab_B + slab_T, 
                                          facecolor='none', edgecolor=beam_color, linestyle='--', linewidth=1.5, zorder=3)
            ax.add_patch(beam_rect)

        # 3. Bottom Edge Beam (Horizontal) -> ถ้า L2_b เป็น 0 (Corner Col)
        if L2_b < 0.1:
             beam_rect = patches.Rectangle((-draw_L, -slab_B), draw_L + draw_R, beam_w, 
                                          facecolor='none', edgecolor=beam_color, linestyle='--', linewidth=1.5, zorder=3)
             ax.add_patch(beam_rect)
             # Label only if not labeled by vertical
             if L1_l > 0.1: 
                ax.text(0, -slab_B + beam_w/2, "EDGE BEAM", color=beam_color, fontsize=8, ha='center', va='center', fontweight='bold')

        # 4. Top Edge Beam (Horizontal)
        if L2_t < 0.1:
             beam_rect = patches.Rectangle((-draw_L, slab_T - beam_w), draw_L + draw_R, beam_w, 
                                          facecolor='none', edgecolor=beam_color, linestyle='--', linewidth=1.5, zorder=3)
             ax.add_patch(beam_rect)
    ...
    if L2_t > 1.0: ax.add_patch(patches.Rectangle((-c1_m/2, L2_t - c2_m/2), c1_m, c2_m, **ghost_props)) 
    if L2_b > 1.0: ax.add_patch(patches.Rectangle((-c1_m/2, -L2_b - c2_m/2), c1_m, c2_m, **ghost_props)) 
...
def draw_elevation_real_scale(h_up, h_lo, has_drop, h_drop_cm, drop_w1, c1_cm, h_slab_cm, 
    ...
    # Support Symbols (Function included internally)
    ...
    # --- EDGE BEAM (CORRECTED SECTION) ---
    if edge_beam_params and edge_beam_params.get('has_beam'):
        beam_h = edge_beam_params['depth_cm'] / 100.0
        beam_w = edge_beam_params['width_cm'] / 100.0
        
        # Logic: ถ้าเป็น Edge Column ปกติเราจะเห็นคานขอบ "ตัดผ่าน" (Cross Section) ที่หัวเสา
        # ตรวจสอบว่า "Edge" อยู่ฝั่งไหน? (L1=0 คือซ้าย, L1_R=0 คือขวา)
        
        # Default: ถ้าไม่มี Cantilever บัง และเป็นขอบซ้าย ให้วาดที่ซ้าย
        beam_drawn = False
        
        # กรณีขอบซ้าย (Typical Edge Column)
        # ตรวจสอบว่า มี Cantilever ซ้ายไหม? ถ้ามี คานขอบจะไม่ใช่ขอบนอกสุด แต่อยู่ที่หัวเสา
        # วาดกล่องที่ "ตำแหน่งเสา" (x=0) แต่ขยับให้ผิวชนขอบนอก
        
        # สมมติ Edge Column อยู่ที่ x=0 (เพราะเรา set coordinate แบบนั้น)
        # คานขอบจะอยู่กึ่งกลางเสา หรือริมเสา? ปกติ Spandrel Beam จะอยู่ริมไล่เข้ามา
        
        # วาดที่ตำแหน่งเสา (Center 0)
        # 1. วาดฝั่งขวา (ถ้า Edge อยู่ขวา) หรือฝั่งซ้าย (ถ้า Edge อยู่ซ้าย)?
        # ดูจาก Input: ถ้าเราเลือก Edge Column และ L1_Left = 0 แสดงว่า Edge อยู่ซ้าย
        # ดังนั้นคานต้องอยู่ที่ x = 0 (หัวเสา)
        
        beam_x_pos = 0 
        
        # ถ้า Cantilever ซ้ายไม่มี -> คานอยู่ซ้ายสุดของเสา (flush left) หรือกลางเสา? 
        # เพื่อความสวยงาม วาดให้ Center ตรงกับเสา หรือเยื้องนิดหน่อย
        # เอาเป็น Center ตรงเสา (x = -beam_w/2)
        beam_x_pos = -beam_w / 2
        
        # Exception: ถ้ามี Cantilever คานก็ยังอยู่ที่เสาอยู่ดี 
        # ดังนั้นวาดที่เสา (x=0) เสมอ ถ้ามีคำสั่ง Has Edge Beam ใน Section นี้
