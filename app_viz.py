...
def draw_smart_dim(ax, p1, p2, text, offset=0.5, orientation='h', color=COLORS['dim_line']):
    """
    Draws a professional engineering dimension line with extension lines.
    """
    x1, y1 = p1
    x2, y2 = p2
...
def draw_plan_view(L1_l, L1_r, L2_t, L2_b, c1_cm, c2_cm, col_loc, has_drop, d_w1, d_w2, cant_params):
    ...
    rect_main = patches.Rectangle((-bound_L, -bound_B), bound_L+bound_R, total_H,
                                  facecolor=COLORS['concrete'], edgecolor=COLORS['concrete_outline'], 
    ...
    if has_drop:
        ...
        ax.add_patch(dp)
        ax.text(-d_w1/2, -d_w2/2 - 0.2, f"Drop {d_w1}x{d_w2}m", color=COLORS['rebar'], fontsize=8)
    ...
    if cant_params['has_left']:
        draw_smart_dim(ax, (current_x, -bound_B), (-bound_L, -bound_B), 
                       f"Cant={cant_L:.2f}", offset=-0.8, orientation='h')
        
    if L1_l > 0:
        draw_smart_dim(ax, (-bound_L, -bound_B), (0, -bound_B), 
                       f"L1(L)={L1_l:.2f}", offset=-0.8, orientation='h')
        
    draw_smart_dim(ax, (0, -bound_B), (bound_R, -bound_B), 
                   f"L1(R)={bound_R:.2f}", offset=-0.8, orientation='h')
                   
    if cant_params['has_right']:
        draw_smart_dim(ax, (bound_R, -bound_B), (bound_R+cant_R, -bound_B), 
                       f"Cant={cant_R:.2f}", offset=-0.8, orientation='h')

    # Vertical Dimensions
    # Calculate offset safely to avoid variable not found errors
    dim_offset_x = -(bound_L + cant_L + 0.8)
    
    draw_smart_dim(ax, (0, 0), (0, bound_T), f"L2(Top)={bound_T:.2f}", offset=dim_offset_x, orientation='v')
    
    if bound_B > c2:
        draw_smart_dim(ax, (0, -bound_B), (0, 0), f"L2(Bot)={bound_B:.2f}", offset=dim_offset_x, orientation='v')

    # View Settings
    draw_L_limit = bound_L + cant_L + 1.5
    draw_R_limit = bound_R + cant_R + 1.5
    
    ax.set_xlim(-draw_L_limit, draw_R_limit)
    ax.set_ylim(-bound_B - 2.0, bound_T + 1.0)
    ax.set_aspect('equal')
    ax.axis('off')
...
def draw_elevation_real_scale(h_up, h_lo, has_drop, h_drop_cm, drop_w1, c1_cm, h_slab_cm, is_roof, far_end_up, far_end_lo, cant_params):
    fig, ax = plt.subplots(figsize=(8, 8))
    
    # Units
    c1 = c1_cm/100.0
    hs = h_slab_cm/100.0
    hd = h_drop_cm/100.0 if has_drop else 0
    ...
    # 2. Slab & Drop
    pts_slab = [
        (x_min, slab_top), (x_max, slab_top), 
        (x_max, slab_bot), (x_min, slab_bot)
    ]
    slab_poly = patches.Polygon(pts_slab, closed=True, facecolor=COLORS['concrete'], edgecolor='black')
    ax.add_patch(slab_poly)
    
    if has_drop:
        pts_drop = [
            (-dw/2, slab_bot), (dw/2, slab_bot), 
            (dw/2, drop_bot), (-dw/2, drop_bot)
        ]
        drop_poly = patches.Polygon(pts_drop, closed=True, facecolor=COLORS['concrete'], edgecolor='black')
    ...
    if not is_roof:
        draw_smart_dim(ax, (0, 0), (0, h_up), f"Up={h_up:.2f}m", offset=dim_x, orientation='v')
    
    draw_smart_dim(ax, (0, -h_lo), (0, 0), f"Lo={h_lo:.2f}m", offset=dim_x, orientation='v')
    ...
    if cant_params['has_left']:
        draw_smart_dim(ax, (-cant_params['L_left'], 0), (0, 0), 
                       f"Cant L={cant_params['L_left']:.2f}", offset=0.5, orientation='h')
                       
    if cant_params['has_right']:
        draw_smart_dim(ax, (0, 0), (cant_params['L_right'], 0), 
                       f"Cant R={cant_params['L_right']:.2f}", offset=0.5, orientation='h')
